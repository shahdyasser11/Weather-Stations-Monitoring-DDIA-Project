...
public class WeatherStation {
    ...
    public static void main(String[] args) throws Exception {
        ...
        props.setProperty(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, "kafka-service:9092"); // defines kafka place

        props.setProperty(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG,
                "org.apache.kafka.common.serialization.StringSerializer"); // convert the key which is a string to
                                                                           // bytes, because kafka sends bytes

        props.setProperty(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG,
                "org.apache.kafka.common.serialization.StringSerializer"); // convert the msg body which is a string to
                                                                           // bytes, because kafka sends bytes
        ...
        while (true) {
            ...
            ProducerRecord<String, String> record = new ProducerRecord<>("weather-status", jsonMsg); // produces kafka
                                                                                                     // record (takes
                                                                                                     // the msg to a
                                                                                                     // certain topic)
